...
\fn{tan}\luastd{math.tan}, \fn{asin}\luastd{math.asin} eccetera --- che come di
consueto lavorano in radianti --- le funzioni esponenziali
\fn{exp}\luastd{math.exp}, \fn{log}\luastd{math.log},
\fn{log10}\luastd{math.log10}, quelle di arrotondamento
\fn{ceil}\luastd{math.ceil}, \fn{floor}\luastd{math.floor}, e quelle per la
generazione pseudocasuale di numeri come \fn{random}\luastd{math.random}, e
\fn{randomseed}\luastd{math.randomseed}. Oltre a funzioni, la tabella include
campi numerici come la costante \( \pi \)\luak{math.pi}.
...
La funzione di base che accetta pattern è \fn{string.match}\luastd{string.match}
