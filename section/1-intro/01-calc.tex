...
Funziona anche con le stringhe a patto di delimitarne il valore, e con i valori
booleani \key{true} e \key{false} e con le stringhe. Proviamo:
\begin{tcolorbox}
...
separato dall'espressione con una virgola:
\begin{Verbatim}
\newcommand\expr[1]{\directlua{
    local result, dec = #1
    if type(result) == "boolean" then
...
\code{\_ENV} di un \emph{chunk}, e anzi, a ben vedere il problema di rendere
...
disponibile nella successiva espressione memorizzato nella nella variabile
\code{ans} come succede con altri tool matematici! Proviamolo:
%
\directlua{
calclib = {}
for name, object in pairs(math) do
...
già stato utilizzato e se non è un un nome di funzione. Inoltre, specificando
una stringa senza chiave tra le opzioni, potremo implementare la memorizzazione
del risultato dell'espressione stessa, così che sia poi riutilizzabile:
\begin{Verbatim}
\directlua{
calclib = {}
for name, object in pairs(math) do
...
la modifica dell'environment, anche per le costanti nelle opzioni della macro è
possibile assegnare espressioni usando tutte le funzioni matematiche e tutte le
costanti precedentemente definite. Da questo punto in poi, possiamo presentare
il valore di \( W_\mathrm{x} \) scrivendo nel sorgente \verb=\expr{w}= che da
\expr{w}.
...
potremo far eseguire calcoli coinvolgendo anche registri contatori o
dimensionali di \TeX{}, oppure considerare che constanti dai nomi speciali come
