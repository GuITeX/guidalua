...
\item \key{table} il tipo tabella \( \to \) capitolo \ref{iiChTabella};
\item \key{boolean} il tipo booleano \( \to \) capitolo \ref{iiChOpLogic};
\item \key{string} il tipo stringa \( \to \) capitolo \ref{iiChStringhe};
\item \key{function} il tipo funzione \( \to \) capitolo \ref{iiChFunzioni}.
\item \key{nil} il tipo nullo \( \to \) sezione \ref{iiSecTipoNil};
\end{compactitemize}
...
Infatti nel capitolo \ref{iiChIteratori} degli iteratori si fa uso
