...
La funzione \fn{string.sub}\luastd{string.sub} estrae la parte di stringa
dell'intervallo tra due indici iniziale e finale forniti come argomento. Se il
secondo indice non è definito l'intervallo si estende fino alla fine della
stringa.

Inoltre, gli indici d'intervallo possono essere negativi intendendo che la
posizione sarà calcolata dalla fine della stringa. Sottolineo che per indice si
intende la posizione del singolo byte perciò questa funzione non è compatibile
con la codifica multibyte Unicode UTF-8. Vediamone qualche esempio:
\begin{lines}
#[run]
-- string.sub (s, i [, j])
local s = "abc123"
print(string.sub(s, -2)) -- estrae le due ultime posizioni
print(string.sub(s, 2, -2)) -- parte centrale
\end{lines}
